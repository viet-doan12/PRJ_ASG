        ...
        <div class="col-md-9 col-lg-10 main-content">
            ...
            <div class="header-container">
                <div>
                    ...
                    <p class="text-muted mb-0">Organize and manage the product categories</p>
                </div>
            ...
            <div class="card">
                <div class="card-header-custom">
                    <!-- Search form -->
                    <form action="${pageContext.request.contextPath}/admin/categories" method="get" class="w-100 me-3">
                    ...
                    <!-- Add button -->
                    <a href="${pageContext.request.contextPath}/admin/categories?action=add" class="btn btn-primary-custom text-nowrap">
                            ...
                            <c:choose>
                                <c:when test="${not empty categoryList}">
                                    <c:forEach var="cat" items="${categoryList}">
                                        <tr>
                                            <td><span class="text-muted fw-medium">#${cat.categoryID}</span></td>
                                            <td>
                                            ...
                                            <td>
                                                <span class="text-muted text-truncate d-inline-block" style="max-width: 350px;">
                                                    ${not empty cat.description ? cat.description : 'No description provided'}
                                                </span>
                                            </td>
                                            <td>
                                                <c:choose>
                                                    ...
                                                    <c:otherwise>
                                                        <span class="badge-inactive"><i class="fa-solid fa-circle-dot me-1"></i> Inactive</span>
                                                    </c:otherwise>
                                                </c:choose>
                                            </td>
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="5" class="text-center py-5">
                                            ...
                                            <span class="text-muted">No categories found matching your query.</span>
                                        </td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
                        ...
                        <nav aria-label="Page navigation">
                            <ul class="pagination justify-content-center mb-0">
                                <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                    <a class="page-link" href="${pageContext.request.contextPath}/admin/categories?action=list&page=${currentPage - 1}&search=${search}" aria-label="Previous">
                                        <span aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                                <c:forEach var="i" begin="1" end="${totalPages}">
                                    <li class="page-item ${currentPage == i ? 'active' : ''}">
                                        <a class="page-link" href="${pageContext.request.contextPath}/admin/categories?action=list&page=${i}&search=${search}">${i}</a>
                                    </li>
                                </c:forEach>
                                <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                    <a class="page-link" href="${pageContext.request.contextPath}/admin/categories?action=list&page=${currentPage + 1}&search=${search}" aria-label="Next">
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
            <div class="modal-header border-bottom-0">
                <h5 class="modal-title fw-bold text-danger" id="deleteModalLabel">
                    <i class="fa-solid fa-circle-exclamation me-1"></i> Confirm Delete
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body text-dark">
                Are you sure you want to delete category "<strong id="deleteCatName"></strong>"? This action cannot be undone and may fail if there are products belongs to this category.
            </div>
            <div class="modal-footer border-top-0">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal" style="border-radius: 8px;">Cancel</button>
                <a id="confirmDeleteBtn" href="#" class="btn btn-danger" style="border-radius: 8px;">Delete Category</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    function confirmDelete(id, name) {
        ...
        var deleteUrl = '${pageContext.request.contextPath}/admin/categories?action=delete&id=' + id;
